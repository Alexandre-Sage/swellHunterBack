...
export default function missingPhoneErrorTest() {
    describe("SIGN UP MISSING PHONE ERROR",function(){
        it("Should handle posted sign up from and send missing phone error", async () => {
            const agent = chai.request.agent(server);
            const responseMessage=process.env.NODE_ENV==="developpment"?"ADD MONGO DOCUMENT ERROR SWITCH":"The phone's field is empty";
